...
const AddMembers: React.FC<{navigation: any}> = ({navigation}) => {
  const {t} = useTranslation();
  const [searchTerm, setSearchTerm] = React.useState('');
  const [addedMembersArray, setAddMembersArray] = React.useState<any>([]);
    ...
    <SafeAreaView style={[appStyle.safeContainer]}>
      ...
      <KeyboardAwareScrollView
        ...
        <View style={[appStyle.p25, appStyle.flex1]}>
          ...
          <View>
            <Text style={styles.titleStyle}>{t('text_searchForPeople')}</Text>
      ...
      <View style={appStyle.p20}>
        <Button onPress={() => {}}>{t('button_createGroup')}</Button>
      </View>
    </SafeAreaView>
  );
};
